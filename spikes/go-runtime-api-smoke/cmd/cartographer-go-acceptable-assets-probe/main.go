...
import (
	...
	"cartographer-go-smoke/internal/acceptableassetsdecode"
	"cartographer-go-smoke/internal/metadatadecode"
)

const (
	metadataEnvVar = "CARTOGRAPHER_METADATA_RPC_HTTP"
	probeName      = "cartographer-go-acceptable-assets-probe"
	...
	targetStateCallMethod   = "XcmPaymentApi_query_acceptable_payment_assets"
)
...
type probeResult struct {
	...
	RequestedMetadataVersion uint32                              `json:"requestedMetadataVersion,omitempty"`
	MetadataVersion          uint8                               `json:"metadataVersion,omitempty"`
	XCMVersion               uint32                              `json:"xcmVersion,omitempty"`
	RuntimeAPICount          int                                 `json:"runtimeApiCount,omitempty"`
	Method                   *methodProbe                        `json:"method,omitempty"`
	ArgsHex                  string                              `json:"argsHex,omitempty"`
	RawReturnHex             string                              `json:"rawReturnHex,omitempty"`
	Decoded                  *acceptableassetsdecode.ReturnValue `json:"decoded,omitempty"`
...
func runProbe(ctx context.Context, client httpDoer, args []string, env map[string]string) (probeResult, error) {
	...
	metadata, err := fetchMetadataAtVersion(ctx, client, cfg.RPC, cfg.MetadataVersion)
	if err != nil {
		if blocked, ok := err.(blockedError); ok {
			return blockedResult(cfg, blocked.Error()), nil
		}
		return probeResult{}, err
	}

	return probeAcceptableAssets(ctx, client, cfg, metadata)
}
...
func configFromArgsEnv(args []string, env map[string]string) (config, error) {
	...
	metadataVersionFlag := flags.Uint("metadata-version", 0, "metadata version to request, for example 16")
	xcmVersionFlag := flags.Uint("xcm-version", 0, "XCM Version argument for XcmPaymentApi")
	...
	if *metadataVersionFlag == 0 {
		return config{}, fmt.Errorf("missing --metadata-version")
	}
	if *metadataVersionFlag > uint(^uint32(0)) {
		return config{}, fmt.Errorf("--metadata-version must fit in u32")
	}
	if *xcmVersionFlag == 0 {
		return config{}, fmt.Errorf("missing --xcm-version")
	}
	if *xcmVersionFlag > uint(^uint32(0)) {
		return config{}, fmt.Errorf("--xcm-version must fit in u32")
	}

	return config{
		RPC:             rpc,
		MetadataVersion: uint32(*metadataVersionFlag),
		XCMVersion:      uint32(*xcmVersionFlag),
	}, nil
}
...
func fetchMetadataAtVersion(ctx context.Context, client httpDoer, endpoint string, version uint32) ([]byte, error) {
	...
	rawResult, err := callState(ctx, client, endpoint, metadataAtVersionMethod, encodeU32Hex(version))
	if err != nil {
		return nil, err
	}
...
func probeAcceptableAssets(ctx context.Context, client httpDoer, cfg config, rawMetadata []byte) (probeResult, error) {
	projection, err := metadatadecode.DecodeRuntimeAPIs(rawMetadata)
	if err != nil {
		return blockedResult(cfg, "decode runtime APIs: "+err.Error()), nil
	}

	result := probeResult{
		Status:                   "ok",
		RequestedMetadataVersion: cfg.MetadataVersion,
		MetadataVersion:          projection.Version,
		XCMVersion:               cfg.XCMVersion,
		RuntimeAPICount:          len(projection.RuntimeAPIs),
	}

	method, ok := metadatadecode.FindRuntimeAPIMethod(projection, targetAPI, targetMethod)
	if !ok {
		result.Status = "blocked"
		result.Error = targetAPI + "." + targetMethod + " not found in runtime APIs"
		return result, nil
	}
	methodProjection := projectMethod(method)
	result.Method = &methodProjection

	argsHex, err := encodeXCMVersionArgHex(methodProjection, cfg.XCMVersion)
	if err != nil {
		result.Status = "blocked"
		result.Error = err.Error()
		return result, nil
	}
	result.ArgsHex = argsHex

	rawReturn, err := callState(ctx, client, cfg.RPC, targetStateCallMethod, argsHex)
	if err != nil {
		return probeResult{}, err
	}
	result.RawReturnHex = hexString(rawReturn)

	decoded, err := acceptableassetsdecode.DecodeReturn(rawReturn)
	if err != nil {
		result.Status = "blocked"
		result.Error = "decode acceptable payment assets return: " + err.Error()
		return result, nil
	}
...
func encodeXCMVersionArgHex(method methodProbe, xcmVersion uint32) (string, error) {
	if method.API != targetAPI || method.Name != targetMethod {
		return "", fmt.Errorf("metadata method is %s.%s, want %s.%s", method.API, method.Name, targetAPI, targetMethod)
	}
	if len(method.Params) != 1 {
		return "", fmt.Errorf("%s.%s param count = %d, want 1", targetAPI, targetMethod, len(method.Params))
	}
	param := method.Params[0]
	if param.Name != "xcm_version" {
		return "", fmt.Errorf("%s.%s param = %q, want xcm_version", targetAPI, targetMethod, param.Name)
	}
	if param.TypeID == 0 {
		return "", fmt.Errorf("%s.%s xcm_version TypeID is missing", targetAPI, targetMethod)
	}
	return encodeU32Hex(xcmVersion), nil
}
...
func encodeU32Hex(value uint32) string {
	var encoded [4]byte
...
func decodeOptionOpaqueMetadata(data []byte) ([]byte, error) {
	...
	case 0:
		return nil, nil
	case 1:
		metadata, err = reader.readBytesVec()
	default:
	...
	if reader.remaining() != 0 {
		return nil, blockedError{message: fmt.Sprintf("decode metadata_at_version Option<OpaqueMetadata>: trailing data after OpaqueMetadata: %d byte(s)", reader.remaining())}
	}
	return metadata, nil
}

func blockedResult(cfg config, message string) probeResult {
	return probeResult{
		Status:                   "blocked",
		RequestedMetadataVersion: cfg.MetadataVersion,
		XCMVersion:               cfg.XCMVersion,
